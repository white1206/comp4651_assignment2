...
**File and Package Layout:** We have placed files in `\src\main\java\hk\ust\comp4651\`. In this folder, you will find two java files which are `HDFSUtils.java` and `HomeWork_2.java`. `HDFSUtils.java` returns a configured FileSystem. In `HomeWork_2.java`, we have defined the function generateTestFilesHDFS which creates random and empty files in HDFS. Driver logic to recreate sandbox, generate data, print BEFORE summary, call your two methods, print AFTER summary.

### **Submission**

You are required to submit only your source code files including `HomeWork_2.java` and `HDFSUtils.java` (into your `assignment-2/` directory). Moreover, you need to submit one screenshot showing the console with **BEFORE and AFTER** summary lines. Please do not include other files or large datasets.
